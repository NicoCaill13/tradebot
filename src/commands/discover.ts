...
async function pullCandidates(): Promise<string[]> {
  ...
  // 2) Screener fallbacks (best-effort; API varies by version)
  ...
  return Array.from(set).slice(0, 80); // cap to keep calls reasonable
}

async function keepMicrocapUS(tickers: string[]): Promise<{ ticker: string; price: number; cap: number; adv: number; }[]> {
...
async function scoreByTrend(cands: { ticker: string; price: number; cap: number; adv: number; }[]): Promise<{
  ticker: string; price: number; cap: number; adv: number; ema20: number; ema50: number; dist20: number; score: number;
}[]> {
  const out: any[] = [];
  for (const c of cands) {
    try {
         
    const hist = await dailyQuotes(c.ticker);

      if (!hist || hist.length < 50) continue;
      const closes = hist.map(h => h.close as number);
      const e20 = ema(closes, 20).at(-1)!;
      const e50 = ema(closes, 50).at(-1)!;
      const dist = (c.price - e20) / c.price; // negative = below price (good pullback)
      let score = 0;
      if (c.price > e20 && e20 > e50) score += 1;      // in uptrend
      // prefer small pullbacks into ema20 (0 to ~2%)
      const pull = Math.max(0, Math.min(1, (-dist) / 0.02));
      score += pull; // 0..1
      // prefer mid range caps within micro-cap (bonus if 50–200M)
